...
st.set_page_config(page_title="About | Lexington Chatbot", page_icon="ℹ️", layout="wide")

# --- Local image -> data URI helper ---
def to_data_uri(path: Path) -> str:
    mime = "image/png" if path.suffix.lower() == ".png" else "image/jpeg"
    b64 = base64.b64encode(path.read_bytes()).decode()
    return f"data:{mime};base64,{b64}"

# --- Files (place these next to about.py) ---
BANNER_PATH = Path("images/Yes4LexWeb_NavBarShort100x1300.jpg")   # 100 (H) x 1300 (W)
RIGHT_LOGO_PATH = Path("images/teamlogo.png")                   # <- your logo file (e.g., PNG)

# Optional: where the logo should link to
LOGO_LINK = "https://youthsteaminitiative.org/index.html"  # or "" to disable click

# --- URIs ---
BANNER_URI = to_data_uri(BANNER_PATH)
RIGHT_LOGO_URI = to_data_uri(RIGHT_LOGO_PATH) if RIGHT_LOGO_PATH.exists() else None
...
st.markdown(
    f"""
    <style>
      /* Image is 100 (H) x 1300 (W) => 1:13 aspect.
         We set banner height = window width / 13 so the whole image shows. */
      :root {{
        --banner-ar: 13;                 /* width / height */
        --banner-h: calc(100vw / var(--banner-ar));
        --banner-pad: 16px;             /* extra spacing below banner */
      }}

      /* One full-width banner behind the entire app (main + sidebar) */
      .stApp::before {{
        content: "";
        position: fixed;
        inset: 0 auto auto 0;
        width: 100vw;
        height: var(--banner-h);
        background: url('{BANNER_URI}') center/cover no-repeat;
        z-index: 0;                     /* keep behind content */
        pointer-events: none;
      }}

      /* Make Streamlit shells transparent so banner shows through */
      ...
      /* Push BOTH main and sidebar content below the banner */
      .block-container {{
        position: relative;
        z-index: 1;
        padding-top: calc(var(--banner-h) + var(--banner-pad));
      }}
      [data-testid="stSidebar"] > div:first-child,
      section[data-testid="stSidebar"] > div {{
        position: relative;
        z-index: 1;
        padding-top: calc(var(--banner-h) + var(--banner-pad)) !important;
      }}
      
        

      /* Right logo overlay (clickable) */
      #global-banner-right-logo {{
        position: fixed;
        top: 0;
        ...
        align-items: center;
        padding-right: 16px;
        z-index: 2;                     /* above banner + content */
        pointer-events: none;           /* let the IMG receive clicks only */
      }}
      #global-banner-right-logo a, 
      #global-banner-right-logo img {{
        pointer-events: auto;
      }}
      #global-banner-right-logo img {{
        height: calc(var(--banner-h) * 0.92); /* scale logo within banner */
        width: auto;
        display: block;
      }}
...
# Make the current page label bigger in the sidebar nav (works on About Us4 when you're on it)
st.markdown("""
<style>
  [data-testid="stSidebarNav"] a[aria-current="page"] span {
    font-size: 8.6rem !important;   /* tweak size */
...
# Inject the right logo element if present
if RIGHT_LOGO_URI:
    if LOGO_LINK:
        st.markdown(
            f"""
            <div id="global-banner-right-logo">
              <a href="{LOGO_LINK}" target="_blank" rel="noopener">
                <img src="{RIGHT_LOGO_URI}" alt="Logo"/>
              </a>
            </div>
            """,
            unsafe_allow_html=True,
...
assistantAvatar = './images/yes4lex.svg'
